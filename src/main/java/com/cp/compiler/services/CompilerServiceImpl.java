...
import static com.cp.compiler.utilities.EntryPointFile.createPythonEntrypointFile;
...
public class CompilerServiceImpl implements CompilerService {
    ...
    @Override
    public ResponseEntity<Response> compile(String sourceCode,
        ...
        if (memoryLimit < minExecutionMemory || memoryLimit > maxExecutionMemory) {
            ...
            return ResponseEntity
                    .badRequest()
                    .body(new Response("Error memoryLimit must be between " + minExecutionMemory + "Mb and " + maxExecutionMemory
                            + "Mb, provided : " + memoryLimit, "Invalid memory limit"));
        ...
        if (timeLimit < minExecutionTime || timeLimit > maxExecutionTime) {
            ...
            return ResponseEntity
                    .badRequest()
                    .body(new Response("Error timeLimit must be between " + minExecutionTime + " Sec and " + maxExecutionTime
                            + " Sec, provided : " + timeLimit, "Invalid time limit"));
        }
        ...
        synchronized (this) {
            ...
            try {
                ...
                // delete source code and entrypoint.sh
                FilesUtil.deleteFile(folder, file);
                FilesUtil.deleteFile(folder, "entrypoint.sh");
        ...
        return ResponseEntity
                ...
                .body(new Response(result.getOutput(), statusResponse));
    }
}
