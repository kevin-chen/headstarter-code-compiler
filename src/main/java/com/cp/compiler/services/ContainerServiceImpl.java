  ...
  public Result runCode(String imageName, MultipartFile outputFile) {
    ...
    return runTimer.record(() -> {
      ...
      try {
        ...
        // Check if the container process is alive, if it's so then destroy it and
        // return a time limit exceeded status
        if (process.isAlive()) {
          ...
          /*
           * Can't get the output from the container (because it did not finish it's
           * execution),
          ...
          return new Result(statusResponse, containerOutput);
        }
      } catch (Exception e) {
        log.error("Error : ", e);
